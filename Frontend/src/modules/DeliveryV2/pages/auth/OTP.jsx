...
export default function DeliveryOTP() {
  ...
  return (
    <div className="min-h-[100dvh] bg-white dark:bg-[#0A0A0B] flex flex-col font-sans overflow-hidden">
      ...
      <div className="relative h-[35dvh] w-full bg-[#EB590E] overflow-hidden flex flex-col items-center justify-center text-white">
        ...
        <div className="max-w-md mx-auto w-full flex flex-col h-full">
          <AnimatePresence mode="wait">
            {!pendingMessage ? (
                        ...
                        <div key={index} className="relative">
                          <input
                            ...
                            className={`w-16 h-20 text-center text-3xl font-black bg-zinc-100 dark:bg-zinc-900 border-2 rounded-2xl text-zinc-900 dark:text-white transition-all outline-none shadow-sm ${
                              focusedIndex === index ? "border-[#EB590E]" : "border-transparent"
                            }`}
                          />
                          {digit && (
                            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-[#EB590E] rounded-full" />
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-6 pt-4 text-center">
                      {resendTimer > 0 ? (
                        <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">
                          Re-pulse in <span className="text-[#EB590E]">{resendTimer}s</span>
                        </p>
                      ) : (
                        <button
                          type="button"
                          ...
                          disabled={isLoading}
                          className="text-xs font-black text-[#EB590E] uppercase tracking-[0.2em] px-6 py-2 rounded-full bg-[#EB590E]/5 hover:bg-[#EB590E]/10 transition-colors"
                        >
                          Resend Pin
                        </button>
                      )}
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl focus-within:border-[#EB590E]/50 focus-within:ring-4 focus-within:ring-[#EB590E]/5 transition-all overflow-hidden">
                          <Input
                      ...
                      {nameError && <p className="text-xs font-bold text-[#EB590E] pl-2">{nameError}</p>}
                    </div>

                    <Button
                      onClick={handleSubmitName}
                      disabled={isLoading || name.trim().length < 2}
                      className="w-full h-16 bg-[#EB590E] hover:bg-[#D94F0C] text-white font-black text-base uppercase tracking-widest rounded-2xl transition-all duration-300 shadow-[0_12px_24px_rgba(235,89,14,0.3)] active:scale-[0.98]"
                    >
                      {isLoading ? (
              ...
              >
                <div className={`w-20 h-20 mx-auto rounded-3xl flex items-center justify-center shadow-xl transform rotate-12 ${isRejected ? "bg-red-50 text-red-600 border border-red-100" : "bg-zinc-50 dark:bg-zinc-900 text-[#EB590E] border border-zinc-100 dark:border-zinc-800"}`}>
                   {isRejected ? <AlertCircle size={40} className="-rotate-12" /> : <ShieldCheck size={40} className="-rotate-12" />}
                </div>

                <div className="space-y-3">
                  <h3 className={`text-xl font-black italic uppercase tracking-tight ${isRejected ? "text-red-600" : "text-[#EB590E]"}`}>
                ...
                <div className="pt-6 flex flex-col gap-4">
                   ...
                   <button 
                    onClick={() => navigate("/food/delivery/login")} 
                    className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.4em] hover:text-[#EB590E] transition-all"
                   >
                    BACK TO BASE
                   </button>
                </div>
          ...
          {error && (
            <motion.div
              ...
              className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-[#EB590E] bg-[#EB590E]/5 py-4 px-4 rounded-2xl border border-[#EB590E]/10"
            >
              <AlertCircle size={14} />
              <span>{error}</span>
            </motion.div>

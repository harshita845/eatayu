...
const PaymentModal = ({ order, otpString, onComplete, onClose }) => {
  ...
  const [isEatAyuingToCash, setIsEatAyuingToCash] = useState(false);
  ...
  useEffect(() => {
    if (paymentStatus === 'pending' && !isCashAccepted) {
      ...
      // Stop polling if we EatAyued away from pending
      if (pollingRef.current) {
        clearInterval(pollingRef.current);
        pollingRef.current = null;
      }
  ...
  const handleTakeCash = async () => {
    setIsEatAyuingToCash(true);
    try {
      await deliveryAPI.EatAyuToCash(orderId);
      setIsCashAccepted(true);
      setPaymentStatus('idle');
      setShowQrModal(false);
      toast.success("EatAyued to Cash Collection");
    } catch (err) {
      toast.error("Failed to EatAyu to cash");
    } finally {
      setIsEatAyuingToCash(false);
    }
  };
            ...
            <div className="bg-amber-50/50 rounded-[2.5rem] p-8 border border-amber-100 mb-8 relative overflow-hidden group">
              ...
              {!isPaid && (
                <div className="space-y-4 relative z-10">
                  <button
                    onClick={generateQr}
                    disabled={isGeneratingQr || isEatAyuingToCash}
                  ...
                  {isCashAccepted ? (
                    ...
                    >
                      {isEatAyuingToCash ? <Loader2 className="w-5 h-5 animate-spin" /> : <DollarSign className="w-5 h-5" />}
                      Take Cash
                    </button>
                  )}

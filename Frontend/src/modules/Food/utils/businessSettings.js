...
const buildThemeOverrideCss = () => {
  ...
  const twSoftBgSelectors = [];
  const twSolidBgSelectors = [];
  const twSolidHoverBgSelectors = [];
  ...
  LEGACY_BRAND_TAILWIND_COLORS.forEach((colorToken) => {
    const isSolid = colorToken.endsWith("500") || colorToken.endsWith("600") || colorToken.endsWith("700") || colorToken.endsWith("800") || colorToken.endsWith("900");
    twTextSelectors.push(`.text-${colorToken}`);
    if (isSolid) {
      twSolidBgSelectors.push(`.bg-${colorToken}`);
      twSolidHoverBgSelectors.push(`.hover\\:bg-${colorToken}:hover`);
    } else {
      twSoftBgSelectors.push(`.bg-${colorToken}`);
      twSoftBgSelectors.push(`.hover\\:bg-${colorToken}:hover`);
    }
  ...
  return `
    ...
    /* Map all common Tailwind blue/indigo button states to primary theme color */
    .bg-blue-600, .bg-blue-500, .bg-indigo-600, .bg-indigo-500, .bg-blue-700 {
      background-color: var(--module-theme-color) !important;
    }
    .hover\:bg-blue-700:hover, .hover\:bg-blue-800:hover, .hover\:bg-indigo-700:hover, .hover\:bg-indigo-800:hover, .hover\:bg-blue-600:hover {
      background-color: rgba(var(--module-theme-rgb), 0.9) !important;
    }
    .text-blue-600, .text-blue-500, .text-indigo-600, .text-indigo-500 {
      color: var(--module-theme-color) !important;
    }
    .hover\:text-blue-700:hover, .hover\:text-indigo-700:hover, .hover\:text-blue-600:hover {
      color: rgba(var(--module-theme-rgb), 0.9) !important;
    }
    ...
    ${twSoftBgSelectors.join(", ")} {
      background-color: rgba(var(--module-theme-rgb), 0.10) !important;
    }
    ${twSolidBgSelectors.join(", ")} {
      background-color: var(--module-theme-color) !important;
      color: #fff !important;
    }
    ${twSolidHoverBgSelectors.join(", ")} {
      background-color: rgba(var(--module-theme-rgb), 0.9) !important;
      color: #fff !important;
    }
 ...
 * @returns {string} Company name or default "EatAyu Food"
 */
export const getCompanyName = () => {
  const settings = getCachedSettings();
  return settings?.companyName || "EatAyu";
};

/**
 * Get company name asynchronously (loads if not cached)
 * @returns {Promise<string>} Company name or default "EatAyu Food"
 */

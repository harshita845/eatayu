...
export default function AdminSidebar({ isOpen = false, onClose, onCollapseChange }) {
  ...
  useEffect(() => {
    if (navRef.current) {
      const savedScrollTop = sessionStorage.getItem("admin_sidebar_scroll_top")
      if (savedScrollTop) {
        // Set immediately
        navRef.current.scrollTop = parseInt(savedScrollTop, 10)
        
        // Also apply after a tiny delay for layout stabilization/accordion expansion
        const id = setTimeout(() => {
          if (navRef.current) {
            navRef.current.scrollTop = parseInt(savedScrollTop, 10)
          }
  ...
  const handleScroll = () => {
    if (navRef.current) {
      sessionStorage.setItem("admin_sidebar_scroll_top", navRef.current.scrollTop.toString())
    }
  }
      ...
      return (
        <div key={index}>
          ...
          {isExpanded && item.subItems && (
            <div className="ml-5 mt-1 space-y-1 border-neutral-800/60 pl-3 overflow-hidden">
    ...
    <>
      ...
      >
        ...
        <nav
          ...
          className="admin-sidebar-scroll flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-3 py-3 space-y-2"
        >

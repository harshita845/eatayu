...
const buildEatAyu99MenuItem = (food, restaurant, restaurantId) => {
...
const buildEatAyu99RestaurantRow = (restaurant, menuItems, index, effectiveLocation) => {
...
export default function Under250() {
  ...
  const isEatAyu99EligibleItem = useCallback((item = {}) => {
  ...
  useEffect(() => {
    const fetchRestaurantsUnder250 = async () => {
      ...
      try {
        ...
        foods.forEach((food) => {
          if (!isEatAyu99EligibleItem(food)) return
        ...
        const restaurantsWithUnder250Dishes = candidateRestaurants
          .map((restaurant, index) => {
            ...
            const menuItems = restaurantFoods.map((food) =>
              buildEatAyu99MenuItem(food, restaurant, restaurantId),
            )

            return buildEatAyu99RestaurantRow(
              restaurant,
              menuItems,
              index,
              effectiveLocation,
  ...
  }, [zoneId, effectiveLocation, filterCandidateRestaurants, isEatAyu99EligibleItem])
  ...
  const updateItemQuantity = (item, newQuantity, event = null, preferredVariant = null) => {
    ...
    const restaurant = item.restaurant || "EatAyu 99"
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: `Check out ${item.name || "this dish"} from ${item.restaurant || "EatAyu 99"}`,
          url: shareUrl,
        })
        return
      }
  ...
  const handleShareOption = async (type) => {
    ...
    const shareUrl = restaurantSlug
      ? `${window.location.origin}/user/restaurants/${restaurantSlug}${itemId ? `?dish=${encodeURIComponent(itemId)}` : ""}`
      : window.location.href
    const shareText = `Check out ${selectedItem.name || "this dish"} from ${selectedItem.restaurant || "EatAyu 99"}`
            ...
            >
              ...
              >
                ...
                <p className="text-sm md:text-base lg:text-lg text-gray-600 dark:text-gray-400 mb-4 md:mb-6 lg:mb-8 leading-relaxed">
                  {selectedItem.description || `${selectedItem.name} from ${selectedItem.restaurant || 'EatAyu 99'}`}
                </p>

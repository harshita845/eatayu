...
export default function HubMenu() {
  ...
  const [EatAyuingOffTarget, setEatAyuingOffTarget] = useState(null) // { type: 'item' | 'group', id: string, groupId?: string }
  ...
  // Refresh menu when page comes into focus (e.g., when user EatAyues from admin panel)
  useEffect(() => {
    const handleFocus = () => {
          ...
          try {
            ...
            try {
              uploadResponse = await uploadAPI.uploadMedia(file, {
                folder: 'EatAyu/restaurant/addons'
              })
...
const toggleItemStock = async (itemId, groupId) => {
  ...
  if (item && item.isAvailable) {
    setEatAyuingOffTarget({ type: 'item', id: itemId, groupId })
...
const handleAvailabilityConfirm = async () => {
  if (!availabilityReason || !EatAyuingOffTarget) return
  ...
  try {
    ...
    if (EatAyuingOffTarget.type === 'item') {
      const payload = buildHubAvailabilityPayload(availabilityReason, customDateTime)
      await restaurantAPI.updateFood(EatAyuingOffTarget.id, payload)
      setMenuData((prev) => markMenuItemUnavailable(prev, EatAyuingOffTarget.id))
    ...
    } else if (EatAyuingOffTarget.type === 'group') {
      const group = menuData.find((entry) => entry.id === EatAyuingOffTarget.id)
      ...
      setMenuData((prev) =>
        prev.map((entry) =>
          entry.id === EatAyuingOffTarget.id
            ? {
              ...entry,
              isEnabled: false,
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${addon.isAvailable === false
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-gray-500 ml-8">
                    This item will not be visible to customers on the Zomato app till you EatAyu it on.
                  </p>
                </div>
              </div>
            </div>
                        ...
                        <div className="space-y-3">
                          {group.items.map((item) => (
                            <div
                              ...
                              className={`flex items-start gap-3 p-3 rounded-lg border border-gray-200 transition-colors ${isPendingApproval(item.approvalStatus)
              ...
              <div>
                ...
                <div className={`grid gap-2 ${isPureVegRestaurant ? "grid-cols-1" : "grid-cols-2"}`}>
                  <button
                    ...
                    className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${addonFoodType === "veg"
                  ...
                  {!isPureVegRestaurant && (
                    <button
                      ...
                      className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${addonFoodType === "non-veg"

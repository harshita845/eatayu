          ...
          {bookings.map((booking) => (
            <div
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <span
                  className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${(booking.status === "confirmed" || booking.status === "accepted")
...
export default function OrdersMain() {
  ...
  const enqueueNewOrder = (rawOrder, { EatAyuToTab = true } = {}) => {
  ...
  useEffect(() => {
    const handleIncoming = (orderData) => {
      if (!orderData) return;
      enqueueNewOrder(orderData, { EatAyuToTab: true });
  ...
  useEffect(() => {
    if (!newOrder) return;
    enqueueNewOrder(newOrder, { EatAyuToTab: true });
      ...
      try {
        ...
        if (response.data?.success && response.data.data?.orders) {
          ...
          for (const orderToQueue of sorted) {
            ...
            enqueueNewOrder(orderForQueue, { EatAyuToTab: false });
          }
        }
      } catch (error) {
        if (error.response?.status !== 401) {
    ...
    <div className="hidden md:flex items-center justify-between px-6 pt-5 pb-3 bg-white">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-2 border rounded-xl hover:opacity-80 transition-all ${onlineStatus === "Online"
              ? "bg-green-50 border-green-100"
              : "bg-gray-50 border-gray-200"
            }`}
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${onlineStatus === "Online" ? "bg-green-500 animate-pulse" : "bg-gray-400"
              }`}
          />
          <span
            className={`text-[12px] font-bold ${onlineStatus === "Online" ? "text-green-700" : "text-gray-600"
              }`}
          >
            {onlineStatus}
          </span>
          <ChevronRight
            className={`w-3.5 h-3.5 ${onlineStatus === "Online" ? "text-green-500" : "text-gray-400"
              }`}
          />
        </button>
    ...
    <div className="w-full bg-white rounded-2xl border border-gray-200 mb-3 overflow-hidden shadow-sm md:hover:border-gray-300 md:transition-colors">
      ...
      <div className="flex items-center justify-between px-3 pt-3 pb-2 gap-2">
        ...
        <div className="flex flex-col items-end gap-0.5 shrink-0">
          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold whitespace-nowrap ${isReady
              ? "bg-green-50 border border-green-400 text-green-700"
              : isPreparing
                ? "bg-amber-50 border border-amber-400 text-amber-700"
                : "bg-gray-100 border border-gray-300 text-gray-700"
            }`}>
            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isReady ? "bg-green-500" : isPreparing ? "bg-amber-500" : "bg-gray-500"
              }`} />
            {statusLabel}
          </span>
      ...
      <div className="flex items-center justify-between gap-2 px-3 py-2 border-t border-gray-100 bg-gray-50/60">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {(isPreparing || isReady || normalizedStatus === "confirmed") && (
            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-semibold ${deliveryPartnerId

...
export default function LandingPageManagement() {
  ...
  const fetchUnder250Banners = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        ...
        const errorMessage = err.response?.data?.message || 'Failed to load EatAyu 99 banners'
        setErrorSafely(errorMessage)
      }
    } finally {
  ...
  const uploadUnder250Banners = async (files) => {
    try {
      ...
      if (response.data.success) {
        setSuccess(`${response.data.data.banners?.length || files.length} EatAyu 99 banner(s) uploaded successfully!`)
        await fetchUnder250Banners()
        setTimeout(() => setSuccess(null), 3000)
      }
    } catch (err) {
      const errorMessage = err.response?.data?.message || 'Failed to upload EatAyu 99 banners'
      setErrorSafely(errorMessage)
  ...
  const handleDeleteUnder250Banner = async (id) => {
    if (!window.confirm('Are you sure you want to delete this EatAyu 99 banner?')) return
    try {
      ...
      if (response.data.success) {
        setSuccess('EatAyu 99 banner deleted successfully!')
        await fetchUnder250Banners()
        setTimeout(() => setSuccess(null), 3000)
      }
    } catch (err) {
  ...
  const tabs = [
    ...
    { id: 'under-250', label: 'EatAyu 99 Banner', icon: Tag },
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              {under250BannersLoading ? (
                ...
                <div className="text-center py-12 text-slate-500">
                  ...
                  <p>No EatAyu 99 banners uploaded yet.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {under250Banners.map((banner, index) => (
                    <div key={banner._id} className="border border-slate-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                      <div className="relative aspect-video bg-slate-100">
                        <img src={banner.imageUrl} alt={`EatAyu 99 Banner ${index + 1}`} className="w-full h-full object-cover" />
                        <div className="absolute top-2 right-2">
                          <span className={`px-2 py-1 rounded text-xs font-medium ${banner.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            {banner.isActive ? 'Active' : 'Inactive'}
                          </span>

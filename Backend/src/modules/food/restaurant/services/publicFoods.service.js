...
export async function listPublicFoods(query = {}) {
    ...
    const isEatAyu99Promo = promo === 'EatAyu99' || promo === 'under-250' || promo === 'under250';
    ...
    const list = await FoodItem.find(foodFilter)
        .sort({ createdAt: -1 })
        .limit(isEatAyu99Promo ? Math.max(limit, 2000) : limit)
        .lean();
    ...
    const foods = list
        ...
        .filter((food) => {
            ...
            if (isEatAyu99Promo) return isEatAyu99Price(food.price);
            return true;
        })
        .slice(0, limit);

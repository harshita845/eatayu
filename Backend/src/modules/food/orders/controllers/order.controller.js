...
export async function EatAyuToCashController(req, res, next) {
    try {
        const deliveryPartnerId = req.user?.userId;
        const orderId = req.params.orderId;
        const result = await orderService.EatAyuToCash(orderId, deliveryPartnerId);
        return sendResponse(res, 200, 'EatAyued to cash collection', result);
    } catch (err) {
        next(err);
    }
}

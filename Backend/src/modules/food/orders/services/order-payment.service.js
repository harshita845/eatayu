...
export async function EatAyuToCash(orderId, deliveryPartnerId) {
  const query = mongoose.Types.ObjectId.isValid(orderId)
    ? { _id: orderId }
    : { orderId };
  ...
  // Only pay-at-delivery orders (legacy COD or QR-collect) may EatAyu to cash.
  const orderPayMethod = String(order.payment?.method || '').toLowerCase();
  if (!['cash', 'razorpay_qr'].includes(orderPayMethod)) {
    throw new ValidationError('Online-paid orders cannot be EatAyued to cash collection');
  }
  ...
  await foodTransactionService.updateTransactionStatus(
    order._id,
    'cod_EatAyued_to_cash',
    {
      recordedByRole: 'DELIVERY_PARTNER',
      recordedById: deliveryPartnerId,
      note: 'Rider EatAyued from QR to Cash collection',
    }
  );
